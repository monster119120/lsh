...
from lsh.lsh_helper import *
import ADSH_PyTorch.models.network as net


def train_one_epoch(task_model, task_optimizer, labeled_dataloader, unlabeled_dataloader, device, cycle, epoch, print_freq):
...
def main(args):
    ...
    hash_mdoel = net.load_model(code_length=96, load=True).to(device)
    ...
    for cycle in range(args.cycles):
        ...
        for epoch in range(args.start_epoch, args.total_epochs):
            train_one_epoch(task_model, task_optimizer, data_loader, unlabeled_dataloader, device, cycle, epoch, args.print_freq)
            task_lr_scheduler.step()
            # vae_lr_scheduler.step()
            # discriminator_lr_scheduler.step()
        ...
        tobe_labeled_inds = lsh_sampling(hash_mdoel, unlabeled_loader, budget_num, device)
